...
describe('GetLatestSectorScoresUseCase', () => {
  ...
  it('returns all scores for the latest date with innerScore and outerScore', async () => {
    const today = new Date('2026-03-19');
    await fake.upsert({ date: today, sector: Sector.TECHNOLOGY, punctualScore: 0.2, structuralScore: 0.3, newsCount: 2 });
    await fake.upsert({ date: today, sector: Sector.ENERGY, punctualScore: -0.2, structuralScore: 0, newsCount: 1 });
    const result = await useCase.execute();
  ...
  it('computes outerScore as cumulative sum of normalized historical scores', async () => {
    ...
    const today = new Date('2026-03-19');
    await fake.upsert({ date: yesterday, sector: Sector.TECHNOLOGY, punctualScore: 0.1, structuralScore: 0.1, newsCount: 2 });
    await fake.upsert({ date: today, sector: Sector.TECHNOLOGY, punctualScore: 0.2, structuralScore: 0.3, newsCount: 2 });
    const [result] = await useCase.execute();
  ...
  it('computes outerScore === innerScore when only one historical snapshot exists', async () => {
    ...
    await fake.upsert({ date: today, sector: Sector.TECHNOLOGY, punctualScore: 0.2, structuralScore: 0.3, newsCount: 2 });
    const [result] = await useCase.execute();
    // Single snapshot: outerScore should equal innerScore = (0.2 + 0.3) / 2 = 0.25
  ...
  it('computes outerScore as sum of two normalized snapshots', async () => {
    ...
    await fake.upsert({ date: day1, sector: Sector.TECHNOLOGY, punctualScore: 0.4, structuralScore: 0.2, newsCount: 4 });
    await fake.upsert({ date: day2, sector: Sector.TECHNOLOGY, punctualScore: 0.6, structuralScore: 0.0, newsCount: 3 });
    const [result] = await useCase.execute();
  ...
  it('returns only latest date scores when multiple dates exist', async () => {
    const yesterday = new Date('2026-03-18');
    const today = new Date('2026-03-19');
    await fake.upsert({ date: yesterday, sector: Sector.TECHNOLOGY, punctualScore: 0.1, structuralScore: 0.2, newsCount: 2 });
    await fake.upsert({ date: today, sector: Sector.TECHNOLOGY, punctualScore: 0.2, structuralScore: 0.3, newsCount: 3 });
    const result = await useCase.execute();
  ...
  it('returns 0 for innerScore when newsCount is 0', async () => {
    const today = new Date('2026-03-19');
    await fake.upsert({ date: today, sector: Sector.TECHNOLOGY, punctualScore: 0.5, structuralScore: 0.5, newsCount: 0 });
    const [result] = await useCase.execute();
